...
export class PushUpdateComponent implements OnInit {
  ...
  triggerEdit() {
    this.AppService.editUpdate(this.editObj.id, this.editObj).subscribe(
      data => {
        this.showEdit = false;
        this.fetchUpdateData(this.status);
      }
  ...
  delete() {
    this.AppService.deleteUpdateRevised(
      this.selectedobj.id,
      this.remarks
    ).subscribe(data => {
      ...
      if (this.status == "Inactive") {
        this.AppService.triggerDeleteOnExpired(this.selectedobj.id).subscribe(
          data => {
            this.showDelete = false;
            this.fetchUpdateData(this.status);
          }
